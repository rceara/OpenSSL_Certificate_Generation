...
## The script we will be using is ** mkcerts-modified.sh ** which is located in the public file repository of this project. The purpose of this script is to generate the required certs for the Cisco WLCs (Wireless LAN Controllers) and the gRPC Collectors. There are different ways to load the trustpoint to the WLCs. We are going to show you how to load them from the CLI by doing a copy/paste of the certs content on the terminal.
